...
<script>
	...
	export default {
		...
		computed: {
			...
			someSensorsHaveInfo() {
				return (
					this.currentSensors.filter((sensor) => this.hasInfo(sensor)).length >
					0
				);
			},
			...
			alertsForCharts(sensorArray) {
				...
				if (this.timeArray.length > 0) {
					...
					alertsForCharts.map((alert) => {
						...
						const closestTimeStart = this.momentTimeArray.reduce(
							(prev, curr) => {
						...
						if (alert.min !== alert.max) {
							const closestTimeEnd = this.momentTimeArray.reduce(
								(prev, curr) => {
