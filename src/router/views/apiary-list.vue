	...
	export default {
					...
					.map((hiveSet) => {
						if (this.filterByReminder) {
							return {
								...
								hives: hiveSet.hives.filter(
									(hive) =>
										hive.reminder !== null || hive.reminder_date !== null,
								),
							};
						} else {
							return hiveSet;
		...
		methods: {
			...
			addDates(hive) {
				...
				const device =
					hive.sensors.length !== 0
						? this.findDeviceById(hive.sensors[0])
						: null;
				const isToday =
					device !== null
						? this.$moment
			...
			getHiveSets() {
				...
				const groupIds =
					this.favGroups.length > 0 ? prefix + this.favGroups : "";
			...
			hiveTagRedirect(hivetags) {
				...
				const filteredHiveTags = hivetags.filter(
					(hiveTag) => hiveTag.tag === tag,
				);
				const hiveTag =
