<template>
  <div>
    <div class="smile-wrapper d-flex align-center ml-1">
      <v-icon
        :class="`${object[property] === 1 ? 'text-red' : 'color-grey'} mr-3`"
        @click="updateObject(1, property)"
      >
        mdi-emoticon-sad
      </v-icon>
      <v-icon
        :class="`${object[property] === 2 ? 'text-orange' : 'color-grey'} mr-3`"
        @click="updateObject(2, property)"
      >
        mdi-emoticon-neutral
      </v-icon>
      <v-icon
        :class="`${object[property] === 3 ? 'text-green' : 'color-grey'} mr-3`"
